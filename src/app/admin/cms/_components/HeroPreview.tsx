...
export default function HeroPreview({ heroContent, featuredProducts }: HeroPreviewProps) {
  ...
  useEffect(() => {
    if (heroContent && heroContent.imagePaths.length > 0) {
      const interval = setInterval(() => {
        setCurrentSlide((prev) => (prev + 1) % Math.ceil(heroContent.imagePaths.length / 2));
      }, 4000);
  ...
  return (
    <>
      <div className="sticky top-0 z-10 mr-4 border-b border-stone-800 bg-white p-4">
      ...
      <div className="p-6">
        ...
        <div className="grid grid-cols-4 gap-0 overflow-hidden rounded-lg border border-gray-200">
          ...
          {/* Empty slots */}
          {Array.from({ length: 4 - featuredProducts.length }).map((_, index) => (
            <div
              key={`empty-${index}`}
              className={`flex h-80 items-center justify-center bg-gray-50 ${
                featuredProducts.length + index < 3 ? 'border-r border-gray-200' : ''
              }`}
            >
