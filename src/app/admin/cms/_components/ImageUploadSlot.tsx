...
interface ImageUploadSlotProps {
  ...
  onUpload: (data: { url: string; publicId: string }) => Promise<{ url: string; publicId: string }> | void;
...
export default function ImageUploadSlot({
  ...
  return (
    <div className="space-y-2">
      {displayImage ? (
        <div
          className={`group relative ${aspectRatio} overflow-hidden rounded-lg border bg-muted transition-all ${
            isDraggingOver ? 'border-primary border-2 ring-2 ring-primary/20' : ''
        ...
        >
          ...
          {showRemoveButton && onRemove && !disabled && (
            <button
              ...
              className="absolute right-2 top-2 rounded bg-destructive p-1.5 text-white opacity-0 transition-opacity hover:bg-destructive/90 focus:opacity-100 group-hover:opacity-100 disabled:cursor-not-allowed disabled:opacity-50"
            >
