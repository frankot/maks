...
export default function HeroTextEditor({ heroContent, onUpdate }: HeroTextEditorProps) {
    ...
    <>
      ...
      <div>
        <div className="mb-3 flex items-center justify-between">
          ...
          <Button variant="ghost" size="sm" onClick={() => setIsEditingText(!isEditingText)}>
        ...
        {isEditingText ? (
          <div className="space-y-3">
            ...
            <div>
              <label className="mb-1 block text-sm font-medium text-gray-700">Link Text</label>
              <Input
                value={textForm.textHref}
                onChange={(e) => setTextForm({ ...textForm, textHref: e.target.value })}
            ...
            <div>
              ...
              <Input
                value={textForm.href}
                onChange={(e) => setTextForm({ ...textForm, href: e.target.value })}
            ...
            <div className="flex space-x-2">
              ...
              <Button variant="outline" onClick={() => setIsEditingText(false)} size="sm">
                <X className="h-4 w-4" />
              </Button>
            </div>
          </div>
          ...
          >
            ...
            <p className="text-xs text-gray-600 transition-colors group-hover:text-blue-500">
              Link: {heroContent?.textHref || 'No link text'} → {heroContent?.href || '#'}
            </p>
          </div>
        )}
      </div>
