...
const fromAddress = process.env.RESEND_FROM_EMAIL || 'MAKS <onboarding@resend.dev>'
...
export async function sendOrderCancellationEmail(email: string, name: string, orderId: string) {
  try {
    const { error } = await resend.emails.send({
      from: fromAddress,
      to: email,
